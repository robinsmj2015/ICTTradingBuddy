...
# ============ Streamlit Layout ============
def render_all(buddy):
    st.title("Trading Buddy Dashboard")
    rec = buddy.recommendation

    col1, col2 = st.columns(2)
    with col1:
        st.plotly_chart(plot_candles(buddy.candles[1].trackers[0].df.tail(15), "1m Price"), use_container_width=True)
        st.plotly_chart(plot_volume(buddy.candles[1].trackers[0].df.tail(15), "1m Volume"), use_container_width=True)
    with col2:
        st.plotly_chart(plot_candles(buddy.candles[5].df.tail(15), "5m Price"), use_container_width=True)
        st.plotly_chart(plot_volume(buddy.candles[5].df.tail(15), "5m Volume"), use_container_width=True)

    st.plotly_chart(plot_candles(buddy.candles[15].df.tail(15), "15m Price"), use_container_width=True)
    st.plotly_chart(plot_speedometer(rec.val if rec.valid else 0, rec.ict_indicators.get("atr", 0)), use_container_width=True)
    ...
    for k in default_keys:
        bars.setdefault(k, 0)
    st.plotly_chart(plot_indicator_bars(bars), use_container_width=True)

    st.plotly_chart(
        plot_balance(buddy.buff.df[buddy.buff.df['actual_profit'].notna()], buddy.trader.starting_balance),
